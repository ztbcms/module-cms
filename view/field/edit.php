...
<div class="wrap J_check_wrap">
    <div id="app" style="margin-bottom: 20px">
        <el-button @click="runBack" type="primary" size="medium">返回</el-button>
    </div>
    ...
    <form name="myform" class="J_ajaxForm" action="{:api_url('/cms/field/edit')}" method="post">
        <div class="table_full">
            <table width="100%" class="table_form">
                    ...
                    <td>
                        ...
                        <select name="formtype" id="formtype" onChange="javascript:field_setting(this.value);">
                            ...
                            {volist name="all_field" id="vo"}
                                <option value="{$key}"
                                        {if condition="$data['formtype'] eq $key" }
                                            selected
                                        {/if}>
                                        {$vo}
                                </option>
                ...
                <tr>
                    ...
                    <td><input type="text" name="field" id="field" size="20" class="input" value="{$data.field}" {if
                               condition=" !$isEditField " } disabled{/if}>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><textarea name="tips" rows="2" cols="20" id="tips"
                                  style="height:40px; width:80%">{$data.tips}</textarea></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>最小值：
                        <input type="text" name="minlength" id="field_minlength" value="{$data.minlength}" size="5"
                               class="input">
                        最大值：
                        <input type="text" name="maxlength" id="field_maxlength" value="{$data.maxlength}" size="5"
                               class="input"></td>
                </tr>
                <tr>
                    ...
                    <td><input type="text" name="pattern" id="pattern" value="{$data.pattern}" size="40" class="input">
                        <select name="pattern" onChange="javascript:$('#pattern').val(this.value)">
                ...
                <tr>
                    ...
                    <td><input type="text" name="setting[backstagefun]" value="{$setting.backstagefun ?? '' }" size="50"
                               class="input">
                        <input name="setting[backstagefun_type]" type="radio" value="1" {if
                               condition="$setting['backstagefun_type'] eq '1'" } checked{/if} >入库前
                        <input type="radio" name="setting[backstagefun_type]" value="2" {if
                               condition="$setting['backstagefun_type'] eq '2'" } checked{/if}>入库后
                        <input type="radio" name="setting[backstagefun_type]" value="3" {if
                               condition="$setting['backstagefun_type'] eq '3'" } checked{/if}>入库前后
                    </td>
                </tr>
                <tr>
                    <th><strong>前台信息处理函数</strong><br/>用法：直接填写函数名称，如果有附带参数可以在函数名后面加###参数1,参数2.完整例子：usfun###a1,a2</th>
                    <td><input type="text" name="setting[frontfun]" value="{$setting.frontfun ?? ''}" size="50"
                               class="input">
                        <input name="setting[frontfun_type]" type="radio" value="1" {if
                               condition="$setting['frontfun_type'] eq '1'" } checked{/if} >入库前
                        <input type="radio" name="setting[frontfun_type]" value="2" {if
                               condition="$setting['frontfun_type'] eq '2'" } checked{/if}>入库后
                        <input type="radio" name="setting[frontfun_type]" value="3" {if
                               condition="$setting['frontfun_type'] eq '3'" } checked{/if}>入库前后
                    </td>
                </tr>
                <tr>
                    <th><strong>值唯一</strong></th>
                    <td><input type="radio" name="isunique" value="1" id="field_allow_isunique1" {if
                               condition="$data['isunique'] eq '1'" } checked{/if} >
                        是
                        <input type="radio" name="isunique" value="0" id="field_allow_isunique0" {if
                               condition="$data['isunique'] eq '0'" } checked{/if} >
                        否
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><input type="radio" name="issearch" value="1" id="field_allow_search1" {if
                               condition="$data['issearch'] eq '1'" } checked{/if}>
                        是
                        <input type="radio" name="issearch" value="0" id="field_allow_search0" {if
                               condition="$data['issearch'] eq '0'" } checked{/if}>
                        否
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><input type="radio" name="isfulltext" value="1" id="field_allow_fulltext1" {if
                               condition="$data['isfulltext'] eq '1'" } checked{/if} />
                        是
                        <input type="radio" name="isfulltext" value="0" id="field_allow_fulltext0" {if
                               condition="$data['isfulltext'] eq '0'" } checked{/if} />
                        否
                    </td>
                </tr>
                <tr>
                    <th><strong>作为万能字段的附属字段</strong><br>必须与万能字段结合起来使用，否则内容添加的时候不会正常显示，使用时直接在使用“{当前字段名}”例如{keywords}</th>
                    <td><input type="radio" name="isomnipotent" value="1" {if condition="$data['isomnipotent'] eq '1'" }
                               checked {/if} <?php if (in_array($data['field'], $forbid_fields)) echo 'disabled'; ?>/> 是
                        <input type="radio" name="isomnipotent" value="0" {if condition="$data['isomnipotent'] eq '0'" }
                               checked {/if}/> 否
                    </td>
                </tr>
                <tr>
                    <th><strong>在推荐位标签中调用</strong></th>
                    <td><input type="radio" name="isposition" value="1" {if condition="$data['isposition'] eq '1'" }
                               checked
                               {/if} /> 是 <input type="radio" name="isposition" value="0" {if
                                                 condition="$data['isposition'] eq '0'" } checked{/if}/> 否
                    </td>
                </tr>
            </table>
        </div>
...
<script type="text/javascript">
    function field_setting(fieldtype) {
        ...
        $.getJSON("{:api_url('/cms/field/public_field_setting')}", {fieldtype: fieldtype}, function (data) {
